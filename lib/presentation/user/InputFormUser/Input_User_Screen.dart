...
class InputUserScreen extends GetView<InputUserController> {
      ...
      appBar: AppBar(
        ...
        actions: [
          ...
          Obx(() {
            ...
            bool isEditing = controller.isEditMode.value; // Ambil status mode edit
            ...
            return Padding(
              ...
              child: TextButton.icon(
                ...
                onPressed: controller.isLoading.value
                    ...
                    : () {
                  Get.defaultDialog(
                    title: "Konfirmasi", // Judul dialog disesuaikan
                    titleStyle: const TextStyle(
                        fontSize: 18, fontWeight: FontWeight.w600),
                    middleText:
                    "Anda yakin ingin ${isEditing ? 'menyimpan perubahan' : 'mengirim jawaban'} form ini?", // Pesan dialog dinamis
                    middleTextStyle: const TextStyle(fontSize: 15),
                    textConfirm: isEditing ? "Ya, Simpan" : "Ya, Kirim", // Teks tombol konfirmasi dinamis
                    textCancel: "Batal",
          ...
          // --- AKHIR MODIFIKASI TOMBOL AKSI APPBAR ---
        ],
            ...
            children: [
              ListView.builder(
                ...
                itemBuilder: (context, index) {
                  if (index == 0) {
                    // --- MODIFIKASI KARTU HEADER FORMULIR ---
                    return Card(
                      elevation: 2.5,
                      margin: const EdgeInsets.only(bottom: 24, left: 4, right: 4, top: 8),
                      shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(16.0)),
                      color: cardBackgroundColor, // Bisa juga Colors.white atau sedikit tinted
                      child: Padding(
                        padding: const EdgeInsets.all(20.0),
                        child: Column(
                          ...
                          children: [
                            Row(
                              ...
                              children: [
                                Icon(
                                  Icons.assignment_ind_outlined, // Ikon disesuaikan (contoh)
                                ...
                                Expanded(
                                  child: Column(
                                    ...
                                    children: [
                                      Text(
                                        form.title, // Contoh: "DC-Penduduk"
                                        style: Get.textTheme.headlineSmall?.copyWith(
                                          fontWeight: FontWeight.bold,
                                          color: accentHeaderColor, // Warna lebih tegas untuk judul
                                          fontSize: 21,
                                        ),
                                      ),
                                      if (form.description.isNotEmpty) ...[
                                        ...
                                        Text(
                                          form.description, // Contoh: "Harap isikan bla bla bla"
                            ...
                            Container(
                              ...
                              decoration: BoxDecoration(
                                  color: primaryHeaderColor.withOpacity(0.15), // Warna latar belakang petunjuk
                                  borderRadius: BorderRadius.circular(10.0),
                    ...
                    // --- AKHIR MODIFIKASI KARTU HEADER ---
                  }

                  // --- KARTU SECTION (BAGAN) YANG BISA DIEKSPANSI ---
                  // (Kode untuk kartu section tetap sama seperti sebelumnya)
  ...
  // ... (Method _buildQuestionsForSection, _buildQuestionLabel, _buildQuestionInput tetap sama seperti sebelumnya) ...
    ...
    return Obx(() {
      ...
      if (repeatIndex != null) {
        if (!controller.repeatableGroupAnswers.containsKey(question.id)) {
          controller.repeatableGroupAnswers[question.id] =
              RxMap<int, dynamic>();
        }
        if (!controller.repeatableGroupAnswers[question.id]!
        ...
        onChangedCallback = (value) => controller
            .updateRepeatableGroupAnswer(question.id, repeatIndex, value);
      } else {
        ...
        onChangedCallback =
            (value) => controller.updateUserAnswer(question.id, value);
      }
      ...
      String? validatorFunction(dynamic val) {
        ...
        if (val is String) {
          ...
          isEmptyAnswer = (val).values.every((colVal) =>
              (colVal as Map).values.every((subColVal) => (subColVal as Map)
                  .values
                  .every((cellVal) =>
              cellVal == null || cellVal.toString().trim().isEmpty)));
        }
        ...
        if (rule != null) {
          ...
          if (currentQuestionState.type == QuestionType.number) {
            num? numAnswer = num.tryParse(effectiveValueString.replaceAll(',', '.')); // handle koma juga
            ...
            if (numAnswer != null) {
              ...
              if (currentQuestionState.code == "203" ||
                ...
                if (artQuestion != null) {
                  final artCountValueDynamic =
                  controller.getAnswerByQuestionId(artQuestion.id);
                  num? artCount;
      ...
      switch (question.type) {
        ...
        case QuestionType.number:
          return TextFormField(
            ...
            keyboardType: const TextInputType.numberWithOptions(signed: false, decimal: false), // Disesuaikan untuk bilangan bulat
        ...
        case QuestionType.checkboxes:
          ...
          List<String>.from(initialValue as List<dynamic>? ?? []);
        ...
        case QuestionType.dropdown:
          ...
          return DropdownButtonFormField<String>(
            key: ValueKey(fieldKeyId +
                (effectiveInitialValue ?? "_nullVal_") +
                optionsToDisplay.join('_')),
            ...
            decoration: _modernInputDecoration(
                ...
                labelText: (optionsToDisplay.isEmpty &&
                    ...
                    : "Pilih salah satu"),
            items: (optionsToDisplay.isEmpty &&
                !isDependent &&
                question.options.isEmpty)
                ? [
          ...
          if (initialValue is Map) {
            try {
              gridAnswers = Map<String, Map<String, Map<String, num?>>>.fromEntries(
                  (initialValue as Map<dynamic, dynamic>)
                      .entries
                      .map((rowEntry) {
                            ...
                            (colMap as Map<dynamic, dynamic>)
                              ...
                              return MapEntry(
                                  ...
                                  Map<String, num?>.fromEntries(
                                      (subColMap as Map<dynamic, dynamic>)
                                          .entries
                                          .map((subColEntry) => MapEntry(
              ...
              print(
                  "Error casting gridAnswers for $fieldKeyId: $e. InitialValue: $initialValue");
            }
          }

          if (question.gridColumnLabels.isEmpty ||
              question.gridSubColumnLabels.isEmpty) {
            return Text(
                "Grid Numerik: Konfigurasi label kolom/sub-kolom belum lengkap.",
                    ...
                    children: [
                      ...
                      Table(
                        ...
                        defaultColumnWidth: const MinColumnWidth(
                            IntrinsicColumnWidth(), FixedColumnWidth(65)),
                        children: [
                          TableRow(
                            decoration:
                                  ...
                                  child: Padding(
                                      ...
                                      child: Text(" ",
                                          style: TextStyle(
                                              fontWeight: FontWeight.bold,
                                              fontSize: 11)))),
                              ...question.gridColumnLabels
                                  .map((colLabel) => TableCell(
                                verticalAlignment:
                              ...
                              children: [
                                TableCell(
                                    ...
                                    child: Padding(
                                        ...
                                        child: Text(subColLabel,
                                            style:
                                            const TextStyle(fontSize: 12)))),
                                ...question.gridColumnLabels.map((colLabel) {
                                  num? cellValue = gridAnswers[rowLabel]
                                  ?[colLabel]?[subColLabel];
                                  String cellKeyIdGrid =
                                      "${fieldKeyId}_grid_${rowLabel}_${colLabel}_$subColLabel";
                                  return TableCell(
                                    verticalAlignment:
                                    TableCellVerticalAlignment.middle,
                                    child: Padding(
                                      ...
                                      child: TextFormField(
                                        ...
                                        initialValue:
                                        cellValue?.toString().replaceAll('.', ',') ?? '', // Tampilkan koma jika ada
                                        textAlign: TextAlign.center,
                                        ...
                                        inputFormatters: [
                                          FilteringTextInputFormatter.allow(
                                              RegExp(r'[\d,]')), // Hanya izinkan angka dan koma
                                        ],
                                        onChanged: (value) {
                                          // Simpan dengan titik sebagai desimal separator
                                        ...
                                        validator: (cellValueString) {
                                          if (cellValueString != null &&
                                              ...
                                              num.tryParse(cellValueString.replaceAll(',', '.')) ==
                                                  null) {
                                            return 'X';
                                          }
                                          return null;
